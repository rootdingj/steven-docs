...
- KEYS[1]：Collections.<Object>singletonList(getName())，表示分布式锁的key，即REDLOCK_KEY；
...
方案  1 中存在的两个问题中“过期时间应该设置多长” ，redisson 的解决方法是：每获得一个锁时，只设置一个很短的过期时间，同时开启一个线程在每次快要到过期时间时去刷新锁的过期时间；在释放锁的同时结束这个线程。
